...
class Agent:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not (self.simulator.game_over()):
                act = self.default_policy(self.simulator.state)
                trajectory.append(self.make_one_transition(act))
            for i in range(len(trajectory)):
                # print(trajectory[i][0])
                # print(trajectory[i][1])
                reward_to_go = 0.0
                for j in range(i, len(trajectory)):
                    reward_to_go += (DISCOUNT ** (j - i)) * trajectory[j][1]
                self.S_MC[trajectory[i][0]] += reward_to_go
                self.N_MC[trajectory[i][0]] += 1
                self.MC_values[trajectory[i][0]] = float(self.S_MC[trajectory[i][0]])/self.N_MC[trajectory[i][0]]
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            # Don't forget the DISCOUNT
            current_s = (self.simulator.state, self.simulator.check_reward())
            while current_s != None:
                act = self.default_policy(self.simulator.state)
                # current_s = (self.simulator.state, self.simulator.check_reward())
                next_s = self.make_one_transition(act)

                if (next_s == None):
                    next_V = 0
                else:
                    next_V = self.TD_values[next_s[0]]

                self.TD_values[current_s[0]] += self.alpha(self.N_TD[current_s[0]]) * (current_s[1] + DISCOUNT*next_V - self.TD_values[current_s[0]])
                self.N_TD[current_s[0]] += 1
                current_s = next_s
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            current_s = (self.simulator.state, self.simulator.check_reward())
            while current_s != None:
                act = self.pick_action(current_s[0], epsilon)
                next_s = self.make_one_transition(act)

                if (next_s == None):
                    next_Q = 0
                else:
                    next_Q = max(self.Q_values[next_s[0]][0], self.Q_values[next_s[0]][1])
                
                self.Q_values[current_s[0]][act] += self.alpha(self.N_Q[current_s[0]][act]) * (current_s[1] + DISCOUNT*next_Q - self.Q_values[current_s[0]][act])
                self.N_Q[current_s[0]][act] += 1
                current_s = next_s
